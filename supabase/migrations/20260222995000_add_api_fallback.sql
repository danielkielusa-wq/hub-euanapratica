-- ============================================================
-- API FALLBACK SYSTEM
-- Adds automatic fallback capability to API configurations.
-- If the primary API fails (credits, rate limit, server error),
-- the system can automatically retry with the fallback API.
-- ============================================================

-- 1. Add fallback_api_key column
ALTER TABLE public.api_configs
  ADD COLUMN IF NOT EXISTS fallback_api_key TEXT
  REFERENCES public.api_configs(api_key) ON DELETE SET NULL;

-- Prevent self-referencing fallback
ALTER TABLE public.api_configs
  ADD CONSTRAINT api_configs_no_self_fallback
  CHECK (fallback_api_key IS DISTINCT FROM api_key);

-- 2. Drop and recreate RPC to include fallback_api_key in RETURNS TABLE
-- (CREATE OR REPLACE cannot change return type, so we must DROP first)
DROP FUNCTION IF EXISTS public.get_api_config_by_key(TEXT);
CREATE FUNCTION public.get_api_config_by_key(p_api_key TEXT)
RETURNS TABLE (
  id UUID,
  name TEXT,
  api_key TEXT,
  base_url TEXT,
  credentials JSONB,
  parameters JSONB,
  description TEXT,
  is_active BOOLEAN,
  created_at TIMESTAMPTZ,
  updated_at TIMESTAMPTZ,
  updated_by UUID,
  fallback_api_key TEXT
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_decrypted_credentials JSONB;
  v_cred_key TEXT;
  v_encrypted_value TEXT;
  v_decrypted_value TEXT;
  v_config RECORD;
BEGIN
  -- SECURITY: Only service_role or admin can access decrypted credentials
  IF current_setting('role', true) IS DISTINCT FROM 'service_role' THEN
    IF NOT has_role(auth.uid(), 'admin'::app_role) THEN
      RAISE EXCEPTION 'Unauthorized: service_role or admin required to access API credentials';
    END IF;
  END IF;

  -- Busca a configuração da API
  SELECT * INTO v_config
  FROM public.api_configs ac
  WHERE ac.api_key = p_api_key
    AND ac.is_active = true;

  IF NOT FOUND THEN
    RETURN;
  END IF;

  -- Se não há credenciais, retorna com JSONB vazio
  IF v_config.credentials IS NULL OR v_config.credentials = '{}'::JSONB THEN
    RETURN QUERY
    SELECT
      v_config.id,
      v_config.name,
      v_config.api_key,
      v_config.base_url,
      '{}'::JSONB as credentials,
      v_config.parameters,
      v_config.description,
      v_config.is_active,
      v_config.created_at,
      v_config.updated_at,
      v_config.updated_by,
      v_config.fallback_api_key;
    RETURN;
  END IF;

  -- Descriptografa cada credencial
  v_decrypted_credentials := '{}'::JSONB;

  FOR v_cred_key, v_encrypted_value IN
    SELECT * FROM jsonb_each_text(v_config.credentials)
  LOOP
    BEGIN
      -- Tenta descriptografar
      v_decrypted_value := decrypt_credential(v_encrypted_value);
      v_decrypted_credentials := v_decrypted_credentials ||
        jsonb_build_object(v_cred_key, v_decrypted_value);
    EXCEPTION WHEN OTHERS THEN
      -- Se falhar descriptografia, usa valor original
      RAISE WARNING 'Failed to decrypt credential % for API %: %. Using original value.',
        v_cred_key, p_api_key, SQLERRM;
      v_decrypted_credentials := v_decrypted_credentials ||
        jsonb_build_object(v_cred_key, v_encrypted_value);
    END;
  END LOOP;

  -- Retorna com credenciais descriptografadas
  RETURN QUERY
  SELECT
    v_config.id,
    v_config.name,
    v_config.api_key,
    v_config.base_url,
    v_decrypted_credentials as credentials,
    v_config.parameters,
    v_config.description,
    v_config.is_active,
    v_config.created_at,
    v_config.updated_at,
    v_config.updated_by,
    v_config.fallback_api_key;
END;
$$;

COMMENT ON FUNCTION public.get_api_config_by_key IS
'Get API config by key with DECRYPTED credentials and fallback info.
SECURITY: Restricted to service_role and admin users only.
Regular authenticated users will receive an authorization error.';

-- 3. Seed defaults: OpenAI ↔ Anthropic cross-fallback
UPDATE public.api_configs SET fallback_api_key = 'anthropic_api' WHERE api_key = 'openai_api';
UPDATE public.api_configs SET fallback_api_key = 'openai_api' WHERE api_key = 'anthropic_api';
