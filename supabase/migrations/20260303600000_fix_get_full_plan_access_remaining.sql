-- Fix: get_full_plan_access ELSE branch hardcoded remaining = GREATEST(0, 5 - cnt)
-- instead of using the plan's features.monthly_credits. This caused the header
-- credit indicator to show wrong values for users without active subscriptions.
...
BEGIN
  ...
  usage_count AS (
    -- UNIFIED: SUM credits_used across ALL apps
  ...
  SELECT
    ...
    uc.cnt,
    -- UNIFIED: monthly_limit reads from features.monthly_credits
    COALESCE((p.features->>'monthly_credits')::INTEGER, 5),
    -- FIX: always use plan's monthly_credits (not hardcoded 5) for remaining calc
