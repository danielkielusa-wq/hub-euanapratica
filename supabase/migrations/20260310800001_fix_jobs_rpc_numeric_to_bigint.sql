-- Fix: salary columns are NUMERIC in the jobs table, not BIGINT.
-- The previous migration changed RETURNS TABLE to BIGINT but didn't cast NUMERIC → BIGINT.
-- This migration adds explicit ::BIGINT casts on the NUMERIC salary columns.
