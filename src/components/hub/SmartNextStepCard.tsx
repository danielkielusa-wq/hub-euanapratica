...
export function SmartNextStepCard({ step }: SmartNextStepCardProps) {
  ...
  return (
    <div className="bg-white dark:bg-card rounded-xl p-6 shadow-sm border border-gray-100 dark:border-white/10 flex flex-col justify-between">
      <div>
        <div className="flex justify-between items-start mb-4">
          <div>
            <h3 className="text-gray-500 dark:text-muted-foreground text-sm font-medium mb-1">Próximo Passo</h3>
            <div className="text-xl font-bold text-gray-800 dark:text-foreground leading-tight">{step.title}</div>
          </div>
          <span className={`text-xs font-bold px-2 py-1 rounded-full whitespace-nowrap ${badgeClasses}`}>
            {badgeLabel}
          </span>
        </div>
        <p className="text-gray-400 dark:text-muted-foreground text-xs mb-4">{step.description}</p>
      </div>
      <button
