...
export default function GlobalLibrary() {
  ...
  // Close mobile folders panel when a folder is selected
  useEffect(() => {
  ...
  return (
    <DashboardLayout>
      <div className="max-w-[1600px] mx-auto lg:h-[calc(100vh-5rem)] flex flex-col font-sans">

        {/* Header */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 mb-4 lg:mb-8">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight flex items-center gap-3">
              <div className="p-1.5 sm:p-2 bg-indigo-600 rounded-lg shadow-lg shadow-indigo-200">
                <Folder className="w-5 h-5 sm:w-6 sm:h-6 text-white fill-white/20" />
              </div>
              Biblioteca
            </h1>
            <p className="text-gray-500 mt-1 ml-11 sm:ml-14 text-sm">
          ...
          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
            ...
            <div className="hidden sm:flex bg-white rounded-xl border border-gray-200 p-1 shadow-sm">
              <button
        ...
        <div className="lg:hidden mb-3">
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-3 bg-white rounded-xl border border-gray-200 shadow-sm text-sm font-medium text-gray-700"
          >
            <span className="flex items-center gap-2">
              <Folder className="w-4 h-4 text-gray-400" />
              {currentFolder
                ? `${currentFolder.icon ? `${currentFolder.icon} ` : ''}${currentFolder.name}`
                : 'Selecione uma pasta'}
            </span>
            {mobileFoldersOpen ? (
          ...
          {mobileFoldersOpen && (
            <div className="mt-2 bg-white rounded-xl border border-gray-100 shadow-sm p-3 max-h-64 overflow-y-auto">
              {sidebarContent}
            </div>
          )}
        </div>

        <div className="flex flex-1 gap-6 lg:gap-8 overflow-hidden">

          {/* Sidebar — desktop only */}
          <div className="hidden lg:flex w-72 flex-shrink-0 flex-col bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6">
              {isBlockedFromFolder && !isSearching ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-center text-gray-400 py-12">
                  ...
                  <p className="text-sm">Navegue pelas pastas para ver os materiais.</p>
                </div>
